...
int owi_configure_reading(void (*input_pulse_callback)(owi_pulse_t), uint8_t new_idle_logic_level, uint8_t use_noise_canceller){
	...
	// Immediately start measuring the second half of a dummy pulse:
	// i.e., behave as if we have already received the terminating pulse of
	// a non-existent previous transmission and measured its width and are
	// now measuring the pulse gap
	input_pulse_firsthalf_width = 0xFFFF;
	// Don't actually measure the gap width, just mark is as overflow, as
	// it does not make sense anyway

	// The "later half of the pulse" is the idle pulse gap
	input_pulse_edgetype = input_idle_logic_level;
	return OWI_OK;
}
